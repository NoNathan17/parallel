...
export function LandingPanel({ apiOk }: LandingPanelProps) {
  return (
    <section
      className="card-glow rounded-2xl px-6 py-10 sm:px-10 sm:py-12"
    ...
    >
      ...
      <ol className="mt-8 grid gap-4 sm:grid-cols-3">
        {[
          ...
          >
            <span
              className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-[var(--primary-soft)] text-sm font-semibold text-[var(--foreground)]"
              aria-hidden
            >
              {item.step}
            </span>
      ...
      {apiOk === false && (
        <p
          role="alert"
          className="mt-6 rounded-lg border border-[var(--error)]/25 bg-[var(--error-bg)] px-4 py-3 text-sm text-[var(--error)]"
        >
          Backend is not reachable. Start it with{" "}
          <code className="rounded bg-[var(--surface-muted)] px-1.5 py-0.5 font-mono text-xs text-[var(--foreground)]">
